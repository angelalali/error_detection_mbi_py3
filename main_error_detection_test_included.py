...
for col in ed.used_cols:
    ...
    if col.strip() in ['Material', 'Plant', 'Plant Description', 'Description', 'Follow-up matl', 'Maintenance status']:
        continue
...
#
    ...
    for row in ed.df[colIndex[col] == False].iterrows():
        ...
        value = row[1][col]
        recommendation = ''
        globalHistogramErrors += [(score, material, col, comment, value, recommendation)]
#
...
if len(ed.df) > 10000:
    ...
    minScore = 0.95  # only consider leaves that have a minimal confidence
...
# test.df = test.df.fillna(value=0)
# ed.df = ed.df.fillna('')
# test.df = test.df.fillna('')

new_columns = ['material', 'plant', 'cell name', 'cell value', 'cognitive score 2', 'comment', 'advice']
all_columns = np.append(ed.df_complete.columns, new_columns)
...
# create one comprehensive data set and then break into train and test, and write out those two sets into cvs
...
# df_all_errors = df_dec_tree_errors
#
# try:
...
#     print('Exception: cannot convert cognitive score to int!')
#
# print('df all error col names', list(df_all_errors.columns.values))
# # df_all_errors.to_csv(config.all_errors_file, index=True, header=df_all_errors.columns.values, columns=all_columns)
# df_all_errors.to_csv(config.all_errors_file, index=False, header=True)
